...
! *************************************************************************
!     code written below: "Perhaps the most common form of packed storage 
!     matrices in quantum chemistry codes involves symmetric matrices". 
!     An N * N packed symmetric matrix includes only the N(N + 1)/2 
!     unique matrix elements. This expression arises when reading in a 
!     symmetric matrix of dimension NDim from a file, where only the upper 
!     or lower triangle of the matrix is stored in the file (since the 
!     other half is redundant due to the symmetry of the matrix.
!     
...
! WRITE CODE HERE TO READ THE ARRAY ELEMENTS FROM THE INPUT FILE.
! *************************************************************************
!     code written below: This code uses a nested loop to assign the 
!     elements of ArrayIn to the corresponding elements of AMatOut in the 
!     lower-triangular format. The outer loop iterates over the columns of 
!     AMatOut (indexed by j), while the inner loop iterates over the rows 
!     (indexed by i). The inner loop is restricted to iterate only over rows 
!     where i = j. Taking note that The storage mode also assumes the 
!     lower-packed storage is packed by columns.
!     
      k = 1 
      do j = 1, N
            do i = j, N
...
! WRITE CODE HERE TO READ THE ARRAY ELEMENTS FROM THE INPUT FILE.
! *************************************************************************
!     code similar to the lower-triangular format except that this loop 
!     fills the upper triangular part of the matrix first
!     
      k = 1 
      do j = 1, N
            do i = 1, j
